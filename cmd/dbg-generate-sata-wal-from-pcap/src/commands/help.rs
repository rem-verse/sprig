...
pub fn handle_help(opt_sub_command: Option<Subcommands>) {
	...
	let (command, is_top_level) = if let Some(sub_command) = opt_sub_command {
		let my_command = subcommands_as_command
			.get_subcommands_mut()
			.find(|potential_command| sub_command.name_matches(potential_command.get_name()))
			.expect(r#"{{"id": "dgswfp::help::internal_error", "cause": "internal error: recognized subcommand could not be matched on name?"}}"#);
		(my_command, false)
	} else {
	...
	info!(
		id = if is_top_level {
			"dgswfp::help::top_level".to_owned()
		} else {
			format!("dgswfp::help::{command_name}")
		},
