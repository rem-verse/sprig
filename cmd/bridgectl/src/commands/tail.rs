...
pub async fn handle_tail(
	...
	let Some((serial_port, path)) =
		coalesce_serial_ports(serial_port_flag.as_ref(), serial_port_positional.as_ref())
	else {
		if SHOULD_LOG_JSON() {
			error!(
	...
	if let Err(cause) = spawn_serial_log_task(serial_port, path).await {
		if SHOULD_LOG_JSON() {
			error!(
