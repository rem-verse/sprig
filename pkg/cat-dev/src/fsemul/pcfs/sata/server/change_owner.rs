...
use crate::{
	fsemul::pcfs::sata::proto::{
		SataChangeOwnerPacketBody, SataPacketHeader, SataResponse, SataResultCode,
	},
...
pub async fn handle_change_owner(
	...
	Body(_packet): Body<SataChangeOwnerPacketBody>,
) -> SataResponse<SataResultCode> {
	SataResponse::new(pid, request_header, SataResultCode::error(0xFFF0_FFE0))
}
...
mod unit_tests {
	...
	pub async fn change_owner_request() {
		...
		let mut response: Bytes =
			handle_change_owner(mocked_header.clone(), State(0), Body(request))
				.await
				.try_into()
