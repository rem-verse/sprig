...
fn raw_decrypt(file_contents: &[u8]) -> Result<Vec<u8>, APIError> {
  ...
  Ok(Vec::from(
    decryptor.decrypt_padded_vec_mut::<NoPadding>(file_contents).map_err(|_| {
      APIError::BadEncryptedDataLength(Aes256EcbDec::block_size())
    })?
  ))
}
...
mod unit_tests {
  ...
  pub fn can_decrypt_and_reencrypt_fw() {
    for (source_file_name, dest_file_name) in vec![
      ...
      let full_encrypted_contents = std::fs::read(&encrypted_path).expect("Failed to read encrypted file to decrypt!");
      let decrypted = raw_decrypt(&full_encrypted_contents[..]).expect("Failed to decrypt data!");
      let expected_decrypted_contents = std::fs::read(&decrypted_path).expect("Failed to read expected decrypted contents!");
